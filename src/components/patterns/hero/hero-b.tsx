        ...
        <aside className="rounded-3xl border border-slate-200/80 bg-white/95 p-6 shadow-[var(--shadow-soft)]">
          ...
          <ul className="mt-4 space-y-2">
            {links.slice(0, 4).map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className="group flex items-center justify-between rounded-xl border border-slate-200/80 bg-slate-50/75 px-3 py-2.5 text-sm font-semibold text-[var(--casa-ink)] transition-colors hover:border-[var(--casa-blue)]/35 hover:text-[var(--casa-accent-text)]"
                >
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            {(spec.chips?.slice(0, 4) ?? []).map((chip) => (
              <span
                key={chip}
                className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-center text-[11px] font-semibold text-slate-600"
              >
                {chip}
              </span>
            ))}
