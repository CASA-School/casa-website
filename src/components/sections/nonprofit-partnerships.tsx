        ...
        <Container>
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            {projects.map(project => (
              <article key={project.name} className="rounded-xl border border-[color:var(--casa-sand)] bg-white p-6 md:p-8">
                <a href={project.href} target="_blank" rel="noopener noreferrer" aria-label={project.name} className={`flex h-24 items-center justify-start rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--casa-blue)] ${project.logo === 'gfh.svg' ? 'bg-[var(--casa-ink-deep)] px-4' : ''}`}>
                  <Image src={`/partners/${project.logo}`} alt={project.name} width={280} height={100} className="h-20 w-full max-w-64 object-contain object-left" />
                </a>
                <p className="mt-6 text-xs font-semibold uppercase tracking-eyebrow text-[var(--casa-accent-text)]">{project.label}</p>
                <h3 className="mt-2 text-xl font-bold">{project.name}</h3>
                <p className="mt-3 text-base leading-relaxed text-[var(--casa-muted)]">{project.text}</p>
                <p className="mt-5 text-sm font-semibold"><a href={project.href} target="_blank" rel="noopener noreferrer" className={linkClass}>{de ? 'Mehr erfahren' : 'Find out more'}<span className="sr-only">: {project.name}</span> ↗</a></p>
                {project.logo === 'gfh.svg' && <p className="mt-3 text-sm"><a href={obsUrl} target="_blank" rel="noopener noreferrer" className={linkClass}>Otto Benecke Stiftung e.V. ↗</a></p>}
              </article>
            ))}
          </div>
          <div className="mt-8 grid gap-6 border-t border-[color:var(--casa-sand)] pt-7 md:grid-cols-2 md:gap-12">
      ...
      <section className="bg-white py-14 md:py-20">
        <Container>
          ...
          <ul className="mt-9 grid grid-cols-2 gap-4 lg:grid-cols-4" aria-label={de ? 'Schulen im TANDEM-Netzwerk' : 'Schools in the TANDEM network'}>
            {schools.map(school => <li key={school.name}><a href={school.href} target="_blank" rel="noopener noreferrer" className="group block h-full rounded-xl border border-[color:var(--casa-sand)] p-4 transition-colors hover:bg-[var(--casa-bg)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--casa-blue)] md:p-6">
              <Image src={`/partners/${school.logo}`} alt={school.name} width={240} height={100} className="h-20 w-full object-contain" sizes="(min-width: 1024px) 240px, 40vw" />
              <p className="mt-5 text-sm font-semibold text-[var(--casa-ink)]">{school.name} ↗</p><p className="mt-1 text-xs text-[var(--casa-muted)]">{!de && school.city === 'München' ? 'Munich' : school.city} · {['Madrid', 'Granada'].includes(school.city) ? (de ? 'Spanien' : 'Spain') : (de ? 'Deutschland' : 'Germany')}</p>
            </a></li>)}
          </ul>
