  ...
  return (
    <section className="overflow-hidden rounded-lg border border-[color:var(--casa-sand)] bg-white shadow-[var(--shadow-card)]">
      <div className="p-5 md:p-7">
        <p className="text-xs font-black uppercase tracking-[0.12em] text-[var(--casa-accent-text)]">{labels?.signature ?? 'Signature'}</p>
        ...
        <div className="mt-5 flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="Course format selector">
          {items.map((item) => (
            <button
              ...
              className={
                selected.id === item.id
                  ? 'shrink-0 rounded-lg border border-[color:var(--casa-ink-deep)] bg-[var(--casa-ink-deep)] px-3.5 py-2 text-xs font-black text-white shadow-[var(--shadow-soft)]'
      ...
      <div className="grid gap-0 border-t border-[color:var(--casa-sand)] lg:grid-cols-[minmax(0,1fr)_minmax(19rem,0.82fr)]">
        <article className="p-5 md:p-7">
          <p className="text-xs font-black uppercase tracking-[0.12em] text-[var(--casa-accent-text)]">{labels?.bestFor ?? 'Best for'}</p>
          <p className="mt-3 text-lg font-black leading-snug text-[var(--casa-ink)]">{selected.bestFor}</p>
          <dl className="mt-5 grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-[color:var(--casa-sand)] bg-[var(--casa-bg)] px-4 py-3">
              <dt className="text-[11px] font-black uppercase tracking-[0.12em] text-[var(--casa-muted)]">{labels?.schedule ?? 'Schedule'}</dt>
              <dd className="mt-1 text-sm font-semibold leading-relaxed text-[var(--casa-ink)]">{selected.schedule}</dd>
            </div>
            <div className="rounded-lg border border-[color:var(--casa-sand)] bg-[var(--casa-bg)] px-4 py-3">
              <dt className="text-[11px] font-black uppercase tracking-[0.12em] text-[var(--casa-muted)]">{labels?.intensity ?? 'Intensity'}</dt>
        ...
        <article className="bg-[var(--casa-warm-soft)]/42 p-5 md:p-7">
          <p className="text-xs font-black uppercase tracking-[0.12em] text-[var(--casa-accent-text)]">{labels?.outcomes ?? 'Likely outcomes'}</p>
