...
export function CourseLevelGoals({
  ...
  const copy = {
    ...
    contextLabel: locale === 'de' ? 'Lehrwerk Context (B1+ – C1)' : 'Context Textbook (B1+ – C1)',
    ...
    <section className="overflow-hidden rounded-3xl border border-[color:var(--casa-sand)] bg-white shadow-[var(--shadow-card)]">
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1.3fr)_minmax(18rem,0.7fr)] border-b border-[color:var(--casa-sand)]">
        ...
        <div className="p-6 md:p-8">
          ...
          <h2 className="mt-2 text-2xl font-black leading-tight text-[var(--casa-ink)] sm:text-3xl">{title}</h2>
        ...
        <aside className="bg-[var(--casa-blue)]/5 p-6 border-l lg:border-l border-[color:var(--casa-sand)] md:p-8 flex flex-col justify-center">
          <p className="text-xs font-black uppercase tracking-[0.12em] text-[var(--casa-accent-text)]">{practiceTitle}</p>
      ...
      <div className="p-6 md:p-8 bg-slate-50/20">
        <div className="grid gap-6 md:grid-cols-2">
          ...
          {netzwerkLevels.length > 0 ? (
            <div className="rounded-xl border border-[color:var(--casa-sand)] bg-white p-5 md:p-6 shadow-[var(--shadow-soft)] flex flex-col">
              <div className="border-b border-[color:var(--casa-sand)]/60 pb-4 mb-4">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-[var(--casa-blue)]/10 px-3 py-1 text-xs font-black uppercase tracking-[0.1em] text-[var(--casa-accent-text)]">
                  {copy.netzwerkLabel}
                </span>
              ...
              <ul className="space-y-4 flex-1">
                {netzwerkLevels.map((item) => (
                  <li key={item.level} className="flex gap-3 items-start">
                    <span className="inline-flex shrink-0 items-center justify-center rounded-lg bg-[var(--casa-accent-surface)] px-2 py-1 min-w-[2rem] text-xs font-black text-white leading-none">
                      {item.level}
                    </span>
          ...
          {contextLevels.length > 0 ? (
            <div className="rounded-xl border border-[color:var(--casa-sand)] bg-white p-5 md:p-6 shadow-[var(--shadow-soft)] flex flex-col">
              <div className="border-b border-[color:var(--casa-sand)]/60 pb-4 mb-4">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-3 py-1 text-xs font-black uppercase tracking-[0.1em] text-amber-800 border border-amber-200">
                  {copy.contextLabel}
                </span>
                <p className="mt-2 text-xs text-[var(--casa-muted)] font-medium leading-relaxed">
                  {copy.contextDesc}
              ...
              <ul className="space-y-4 flex-1">
                {contextLevels.map((item) => (
                  <li key={item.level} className="flex gap-3 items-start">
                    <span className="inline-flex shrink-0 items-center justify-center rounded-lg bg-amber-700 px-2 py-1 min-w-[2rem] text-xs font-black text-white leading-none">
                      {item.level}
                    </span>
                    <p className="text-sm font-semibold leading-relaxed text-[var(--casa-ink)] pt-0.5">
                      {item.focus}
