 ...
 *
 * Added 2026-08-20, after two rounds of work that improved copy and then visual
 * detail while leaving every course page the same shape — measured identically at
 * a 700px/620px body grid on all seven. The archetypes varied section order and
 * permitted facts; they did not vary the page. These assert the dimension that
 * was missing, and the fact lists that made the quote formats look broken.
 */
describe('course page architecture', () => {
  it('gives every archetype a layout', () => {
    for (const archetype of Object.values(courseArchetypes)) {
      expect(archetype.layout, archetype.id).toBeTruthy();
    }
  });

  it('reserves the facts rail for archetypes with real facts', () => {
    expect(courseArchetypes['scheduled-cohort'].layout).toBe('facts-rail');
    expect(courseArchetypes['module-catalogue'].layout).toBe('catalogue');
    expect(courseArchetypes['professional-track'].layout).toBe('brief');
    expect(courseArchetypes['package-inquiry'].layout).toBe('brief');
  });

  /*
   * docs/COURSE_FACTS_SOURCE_OF_TRUTH.md: German for Medical publishes "not
   * published / not published / not published / B2 and C1 entry". This archetype
   * used to declare the whole scheduled-cohort fact set, so the page asserted a
   * next start date and a price for a course that has neither, and four of its
   * five rail rows rendered "On request" or "To be announced".
   */
  it('lets professional-track publish only the level range', () => {
    expect(courseArchetypes['professional-track'].facts).toEqual(['level-range']);
    expect(courseArchetypes['professional-track'].showsStartDateSelector).toBe(false);
  });

  it('never offers a start-date selector on an archetype with no dates', () => {
    for (const archetype of Object.values(courseArchetypes)) {
      if (archetype.showsStartDateSelector) {
        expect(archetype.facts, `${archetype.id} offers dates`).toContain('next-start');
      }
    }
  });

  it('quotes rather than prices on every brief layout', () => {
    for (const archetype of Object.values(courseArchetypes)) {
      if (archetype.layout !== 'brief') continue;
      expect(archetype.facts, `${archetype.id} must not price`).not.toContain('price');
      expect(archetype.cta, `${archetype.id} cta`).not.toBe('register-or-place');
    }
  });
});
