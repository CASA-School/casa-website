...
function readLayout(route: string) {
  ...
  return readFileSync(path.resolve(process.cwd(), 'src/app/(site)/[locale]', route, 'layout.tsx'), 'utf8');
}
...
describe('internal surface route layouts', () => {
  it.each(guardedRoutes)('src/app/(site)/[locale]/%s/layout.tsx still calls the guard', (route) => {
