...
export async function POST(request: NextRequest) {
  ...
  if (webhookUrl) {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Webhook rejected exam registration with status ${response.status}`);
      }
    } catch (error) {
      console.error('[exam-registration-api] submission failed', error);
      return NextResponse.json(
        {
          status: 'error',
    ...
    console.info('[exam-registration-api] accepted request (preview mode)', {
      requestId,
      submittedAt,
